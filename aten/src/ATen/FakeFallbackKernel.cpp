...
at::Tensor to_real_tensor(const at::Tensor& t) {
  ...
  return at::empty_strided(t.sizes(), t.strides(), t.options().device(device))
      .zero_();
}
...
void fakeFallback(
  ...
  // Try decompositions before the meta kernel. Skip ops with a Python meta
  // registration; those match on the meta kernel, matching Python
  // FakeTensorMode's `op not in meta_table` decomp gating. The unbacked-special
  ...
  if (mode && !cpp_meta_supports_symint(op) &&
    ...
    if (interp &&
      ...
      if ((*interp)->fake_try_decomp(op, stack, has_symints)) {
        wrap_meta_outputs_with_default_device_logic();
        return;
      }
    }

    // Run the CIA decomposition by calling its kernel directly, same as python
    // in torch/_ops.py. Only under symbolic sizes: with static shapes a CIA op
    // without a meta kernel already decomposes via the meta dispatch below.
    if (has_symints && !op.hasKernelForDispatchKey(c10::DispatchKey::Meta) &&
  ...
  try {
    // Exclude MkldnnCPU too: a cpp fake converted from an mkldnn tensor carries
    // the MkldnnCPU key (set_fake_mkldnn) so is_mkldnn/layout report correctly.
    // With Fake excluded here, that key would outrank Meta and route to the real
    // oneDNN kernel; exclude it so the Meta kernel runs on the meta backing.
    c10::impl::ExcludeDispatchKeyGuard guard(
        c10::DispatchKeySet(c10::DispatchKey::Fake) |
        c10::DispatchKeySet(c10::DispatchKey::Python) |
        c10::DispatchKeySet(c10::DispatchKey::PythonTLSSnapshot) |
