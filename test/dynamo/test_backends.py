...
class TestCustomBackendAPI(torch._dynamo.test_case.TestCase):
    ...
    def test_dynamo_backend_init_method_dedup(self):
        ...
        # access builds a fresh bound-method object (transient), so dedup keys on
        # __self__ (the class). Without that, calls==2.
        calls = []
    ...
    def test_dynamo_backend_init_aot_autograd(self):
        ...
        # same fw_compiler fire once; and it is read at fire time (@property), so
    ...
    def test_dynamo_backend_init_per_instance(self):
        # An instance-method hook dedups per instance: two backend instances each
        # fire once, so per-device init works. The key is __self__ (the instance),
        # not __func__ (which is shared across the class).
        calls = []
    ...
    def test_dynamo_backend_init_unhashable_hook(self):
        # An unhashable hook callable (e.g. a @dataclass instance) cannot live in
        # the WeakSet; it skips dedup and fires per resolution rather than crashing.
