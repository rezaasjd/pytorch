...
def _capture(
    ...
    # Trace under the CALLER's ambient grad mode. An earlier version forced
    # enable_grad() so a backward inside ``fn`` would be built as graph ops, on the
    # theory that the forward graph is the same either way -- it is not, because ``fn``
    # can read torch.is_grad_enabled() and branch on it, and forcing the flag baked the
    # wrong branch on the very first call in the capturing process. A ``fn`` that runs
    # .backward() under an ambient no_grad now fails exactly as it does in eager.
