...
void ConcretePyInterpreterVTable::propagate_real_tensors(
    ...
    torch::jit::Stack* stack) const {
  py::gil_scoped_acquire gil;
  // Exclude Fake while the driver runs the op on the real tensors. C++ fakes
  // carry the Fake key per-tensor, so without excluding it a fake-keyed arg
  // would re-enter this fallback and recurse. The guard restores it on scope
  // exit (we run mid-fallback, still inside the user's fake-mode scope).
  c10::impl::ExcludeDispatchKeyGuard exclude_fake(
      {c10::DispatchKeySet(c10::DispatchKey::Fake)});
  auto active = get_active_fake_mode();
